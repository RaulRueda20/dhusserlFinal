    ...
    <div className="contenedorMenuDerecho">
      <ExpansionPanel square expanded={expanded1} onChange={() => setExpanded1(!expanded1)} className="panelPrincipal">
        ...
        <ExpansionPanelDetails className="panelDeDetalleExpresion">
          <Typography letiant="caption" className="tagsMenuDerecho">{menuDerechoJerarquiaExpresion(lang)}</Typography>
          <ul className="ulDelMenuDerechoExpresion">
            <li>
              <Typography letiant="h6" className="consultaDePasajes">{expresionSeleccionada?.expresion}</Typography>
            </li>
          </ul>
        </ExpansionPanelDetails>
        <ExpansionPanelDetails className={classNames([{ "panelDeDetalleHijos": listaVerTambien != "" }, "panelDeDetalleHijosLibres"])}>
          <Typography letiant="caption" className="tagsMenuDerecho">{menuDerechoJerarquiaExpresionesDerivadas(lang)}</Typography>
            ...
            <ul className="ulDelMenuDerechoVerTambien">
              {listaVerTambien.map((expresion, index) => {
                return <li key={expresion.id + "-" + index}>
                  <Link to={`${match.path.slice(0, 20)}/pasaje/${expresion.id}`} onClick={(event) => handleFlagLetraMain(event)}>
                    <Typography className={"consultaDePasajes"} letiant="h6" id={expresion.id + "/" + index}>{expresion.expresion + "  //  " + expresion.traduccion + "  --  " + expresion.id}</Typography>
                  </Link>
                </li>
              })}
            </ul>
        ...
        <ExpansionPanelDetails className="panelDeDetalleReferenciasConsultadas">
          <ul className="ulDelMenuDerechoReferenciasConsultadas">
            {referenciasConsultadasVista.map((consultas, index) => {
              return (
                <Link to={`${match.path.slice(0, 20)}/pasaje/${consultas.id}/${consultas.referencias[0].refid}`} onClick={(event) => handleFlagLetraMain(event)} key={consultas.referencias[0].refid + "-" + index}>
                  <li className="bordeDeConsultas" key={consultas.referencias[0].refid + "-" + index}>
                    <Typography className="consultaDePasajes" letiant="h6" id={consultas.id + "/" + index}>{consultas.expresion + "  :  " + consultas.referencias[0].referencia_original + "/" + consultas.referencias[0].referencia_traduccion}</Typography>
                  </li>
                </Link>
              )
            })}
