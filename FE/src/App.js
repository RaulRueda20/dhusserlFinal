...
const App = () => {
  ...
  const handleAlertClose = () => {
    setMensajeDeAlerta({ open: false, mensaje: mensaje, tituloAlerta: tituloAlerta })
  }
  ...
  useEffect(() => {
    if (sesion == null && localStore.getObjects("sesion")) {
      console.log('CASO A')
      var sesionBuscador = { usuario: localStore.getObjects("sesion").user, password: localStore.getObjects("sesion").user }
      dispatch({
        type: "SET_SESION",
        payload: sesionBuscador
      })
    }
    if (ultimasVisitadas.length == 0 && localStore.getObjects("referenciasConsultadas")) {
      dispatch({
  ...
  return (
    <LangProvider>
      ...
      <Alerts mensaje={mensaje} open={open} handleClose={handleAlertClose} titulo={tituloAlerta} />
      <Snackbars
        snackbar={snackbar}
