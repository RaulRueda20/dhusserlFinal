  ...
  res.locals.connection
    ...
    .then((results) => {
      ...
      if (results.length < 1) {
        ...
        bcrypt.compare(password, results[0].user_password, function (
          err,
          res2
        ) {
          if (res2) {
...
router.get("/eraseRegister/:email", function (req, res) {
  if (req.params.email == "azirionq" || req.params.email == " guest") {
    ...
    res.locals.connection
      ...
      .then(function (results) {
        console.log(results);
        bcrypt.compare(req.query.pass, results.user_password, function (
          err,
          res
        ) {
          if (res) {
...
router.get("/sendRegistroEmail/:lang", function (req, res) {
  ...
  switch (req.params.lang) {
    case "es":
      ...
      var here = "aquí";
      var warning =
        "Si usted no ha realizado ningún registro, por favor haga click ";
    ...
    case "al":
      ...
      var here = "hier";
      var warning =
        "Haben Sie keinen Registrierungversuch vollzogen? Bitte klicken Sie ";
    ...
    case "en":
      ...
      var here = "here";
      var warning =
        "If you have not made any attempt to register, please click ";
    ...
    case "fr":
      ...
      var here = "aquí";
      var warning =
        "Si usted no ha realizado ningun registro, por favor haga click ";
      var warning2 = " para deshacer el registro.";
      var subj = "¡Registro exitoso!";
      var nameCC = "Servicio de Registro DH";
      break;
  ...
  // var htmlContent = "<h3>" + header + "</h3><hr/><br/><form><p>" + body + "</p><p>" + warning + " <a href='" + serverUrl + "/" + toEmail + "&pass=" + password + "'>" + here + "</a> " + warning2 + "</p></form>" + foot
  var htmlContent =
    "<h3>" + header + "</h3><hr/><br/><form><p>" + body + "</p></form>" + foot;
  ...
  transporter.sendMail(mailOptions, (error, info) => {
    ...
    res.send(
      JSON.stringify({ status: 200, error: null, response: "Envio exitoso" })
    );
    ...
    .then(function (results) {
      ...
      if (results.length > 0) {
        ...
        var htmlContent =
          ...
          here +
          " " +
          "http://localhost:8080/#/diccionario/recuperacionPass/" +
          emailEncoded +
          "</p>\
            <p>" +
          warning +
        ...
        transporter.sendMail(mailOptions, (error, info) => {
          if (error) {
            res.send(
              JSON.stringify({ status: 500, error: null, response: null })
            );
            return console.log(error);
          }
          res.send(
